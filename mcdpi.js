...
let mc = {}
mc.__meta = {mode: MODE_DEFAULT, writing: false, root: "", curFunc: {path: "", name: ""}, pack: {name: "", desc: "", format: 0}, namespace: "", hooks: {}, lines: {}, markers: [], marks: {}}
...
mc.startPack = function(name, desc = "", format = 5)
{
	// if (mc.__meta.root != "")
...
// function argument if you need to dabble
...
mc.addObjective = function(name, criteria = "dummy", displayName = false) {mc.command("scoreboard objectives add " + name + " " + criteria + (displayName ? " " + displayName : ""))}

mc.setScore = function(a, b, c = "nope")
{
	if (c != "nope")
...
mc.addScore = function(a, b, c = "nope")
{
	if (c != "nope")
...
mc.subtractScore = function(a, b, c = "nope")
{
	if (c != "nope")
...
mc.execute = function(txt = "execute")
{
	...
	{
		mc.addPredicate("percentage_" + pct, "random_chance", ["chance"], [pct / 100])
		return mc.execute(txt + " if predicate " + mc.__meta.namespace + ":percentage_" + pct)
	}
...
mc.hook = function(name, func)
{
	let pack = mc.__meta.pack.name
	let namespace = mc.__meta.namespace
	mc._mkdir(pack + "/data/minecraft")
	mc._mkdir(pack + "/data/minecraft/tags")
	mc._mkdir(pack + "/data/minecraft/tags/functions")
	...
	mc._write(pack + "/data/minecraft/tags/functions/" + name + ".json", '{"replace":"false","values":[' + mc.__meta.hooks[name].toString() + ']}')
}
